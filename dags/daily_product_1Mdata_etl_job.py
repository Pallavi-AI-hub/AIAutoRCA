...
def load_product_csv():
    ...
    logger.info("Starting 1M load — SLA threshold: %ds", SLA_SECONDS)
    ...
    try:
        for chunk in pd.read_csv(PRODUCT_FILE, chunksize=chunk_size):
            ...
            execute_values(
                cur,
                """
                INSERT INTO product_details_master (
        ...
        logger.info("1M load complete — %d rows | total elapsed=%.1fs",
...
def export_product_csv():
    ...
    conn = get_conn()
    df   = pd.read_sql("SELECT * FROM product_details_master", conn)
    conn.close()
...
with DAG(
    ...
    load_task   = PythonOperator(task_id="load_1m_data", python_callable=load_product_csv, on_failure_callback=on_failure_callback)
