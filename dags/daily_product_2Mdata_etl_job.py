...
PRODUCT_FILE  = "/home/mpallavi/RETAIL_AI_RCA/data/retail_product_data_2M.csv"
EXPORT_FILE   = "/home/mpallavi/RETAIL_AI_RCA/data/product_master_2M_export.csv"
...
def load_product_csv():
    ...
    DAG_ID        = "Daily_product_2Mdata_ETL_job"
    TASK_ID       = "load_2m_data"
    start         = _time.time()
    sla_triggered = False
    inserted      = 0
    chunk_size    = 10000
    ...
    logger.info("Starting 2M load — SLA threshold: %ds", SLA_SECONDS)
    ...
    try:
        for chunk in pd.read_csv(PRODUCT_FILE, chunksize=chunk_size):
            ...
            execute_values(
                cur,
                """
                INSERT INTO product_details_master_2M (
        ...
        logger.info("2M load complete — %d rows | total elapsed=%.1fs",
...
def export_product_csv():
    ...
    conn = get_conn()
    df   = pd.read_sql("SELECT * FROM product_details_master_2M", conn)
    conn.close()
...
with DAG(
    ...
    load_task   = PythonOperator(task_id="load_2m_data", python_callable=load_product_csv, on_failure_callback=on_failure_callback)
